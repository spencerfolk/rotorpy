...
controller = BatchedSE3Control(quad_params, num_drones=num_quads, device=torch.device("cpu"))
...
observation = torch.from_numpy(env.reset(options={'initial_state': 'random'}))
...
for i in range(T):
    ...
    action = np.interp(cmd_motor_speeds, [env.unwrapped.rotor_speed_min, env.unwrapped.rotor_speed_max], [-1,1]).reshape(num_quads, 4)
